...
the White House moved their Revocery.gov~\cite{recovery} operation to
...
Twitter is a good example of a service that can have difficulties estimating amount user demand 
and total amount of incoming requests.
For instance in the year $2011$ they achieved a record in \myac{TPS} of
$25,088$, each ``tweet'' essentially becomes at least one request to Twitter services.
...
supporting familiar protocols such as HTTP/HTTPS and \myac{SSH}.
...
instances are given more resources such as RAM and CPU.
...
The layers refer to different levels of abstractions
ways of 
...
Examples of providers of IaaS are AWS \myac{EC2} and Rackspace CloudServers.
...
This database is transparently interfaced using their API, 
but also support technologies such as JPA and JDO,
...
different technologies such as \myac{REST} APIs or SOAP services, but in any case it is 
...
\myac{MDA} is a way of designing software with modeling in mind provided by \myac{OMG}.
When working with MDA it is common to first create a \myac{CIM}, then a \myac{PIM} and
...
\begin{enumerate}
  ...
  \item \emph{Convert the PIM into PSM.}
    ...
    \epigraph{
      A PSM is not a PIM, but is also not an implementation. [\ldots]
