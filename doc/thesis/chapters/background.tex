...
For instance in the year $2011$ they achieved a record of $25,088$ \myac{TPS},
triggered by a Japanese television screening of the movie \emph{``Castle in the Sky''}.
Each \emph{``tweet''} essentially becomes at least one request to Twitter services.
...
such as programming languages (\eg Java, C\#), environments (\eg \myac{JVM}, .NET, Node.js), 
storage systems (\eg flat files, NoSQL databases, \myac{RDBMS}),
...
Examples of \myac{PaaS} providers are Google with \myac{GAE} and
the company Heroku with their eponymous service.
Amazon also entered the \myac{PaaS} market with their service named Elastic Beanstalk,
...
\myac{PaaS} providers support deployments through online \myac{API}s, in many cases by providing 
...
The definition of \emph{``modeling''} can be assessed from the previous epigraph, but it is 
...
\begin{enumerate}
  ...
  \item \emph{Convert the \myac{PIM} into \myac{PSM}.}
    ...
    From this it is possible to determine that a \myac{PSM} is more specific to a platform than \myac{PIM}, such as 
