...
When working with \emph{Model-Driven Architecture}~(MDA) it is normal to first create a
\emph{Computation Independent Model}~(CIM), then a \emph{Platform-Independent Model}~(PIM) and
lastly a \emph{Platform-Specific Model}~(PSM). 
There are five different life as explained by cycles~\cite{model-driven:singh09}:
\begin{enumerate}
...
specific to CloudML and what is out-scoped.
