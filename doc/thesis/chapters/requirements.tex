...
All of the API libraries from \citechap{state-of-the-art} support \emph{multicloud} so they can
...
\begin{description}
  \item[Java.] Standard Java classes and expect users to do \emph{short polling} (client 
  must poll for updates at a set interval, \eg every 5 seconds).
  \item[Command pattern.] Send events on node updates.
  \item[Actor model.] Enhance asynchronous support and modularity.
  \item[Publish subscribe pattern.] Let users subscribe to events of updating instances.
\end{description}
...
Since \citereq{foundation} is a central dependency (\citefig{requirement-dependencies})
