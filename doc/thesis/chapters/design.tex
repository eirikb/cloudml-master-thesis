...
The roles Alice assumes in this scenario, regarding \citefig{big-picture},
...
This is not an uncommon topology.
...
In Alices case the node has increased two important attributes to support both higher computation 
demand and storage capabilities, \ie $2$ cores and $2000$ \myac{GB}
\footnote{
  Size in \myac{GB}, but measurement can change depending on implementation of \citereq{software-reuse}.
}
in disk size.
...
\eg if Alice deployed an application that should suddenly, 
