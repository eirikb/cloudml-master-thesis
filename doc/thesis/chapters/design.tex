...
demand and storage capabilities, \ie, $2$ cores and $2000$ in disk size (\myac{GB} respectively).
...
The \texttt{build} method support provisioning several templates to one account (same provider),
but in this scenario that is not utilized.
There is also a constraint that a set of templates can not be simultaneously 
cross-deployed to different providers. They can, however, be re-provisioned sequentially
